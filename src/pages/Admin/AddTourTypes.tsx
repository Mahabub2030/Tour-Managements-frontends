...
import { AddTourModal } from "@/components/modules/Authentication/Admin/TourType/AddTourModal";
...
const handelRemoveTourType = async(tourId:string)=>{
    const toastId = toast.loading("Removeing........")
    try {
          ...
          <TableBody>
            {data?.data?.map((item: {_id:string, name: string }) => (
              <TableRow>
                ...
                <TableCell>
                  <DeleteConfirmation onConfrim={()=>handelRemoveTourType(item._id)}>
                    <Button size="sm">
                      <Trash2 />
                    </Button>
                  </DeleteConfirmation>
